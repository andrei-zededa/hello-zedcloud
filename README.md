...
One such additional feature is the `-bw-limit` CLI flag. With it the server can
...
any files in the static directory:
  - `/echo`      - Get back a dump of the HTTP request.
  - `/upload`    - Accepts a multi-part file upload and saves the uploaded file
